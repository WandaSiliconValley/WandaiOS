...
class ClassesTableViewCell: UITableViewCell {
    ...
    let shortMonthDateFormatter: DateFormatter = {
        let formatter = DateFormatter()
    ...
    func configureClass(_ wandaClass: WandaClassInfo) {
        guard let eventDate = simpleDateFormatter.date(from: wandaClass.date) else {
            return
        }

        let calendar = Calendar.current
        let month = shortMonthDateFormatter.string(from: eventDate)
